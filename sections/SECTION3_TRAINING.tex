...
embeddings \cite{fan2025alignformer,bai2024seed}. Modality alignment
...
\cite{radford2023robust}, HuBERT \cite{hsu2021hubert}, wav2vec~2.0
\cite{baevski2020wav2vec}) with a partly frozen text LLM through a
trainable bridge. \emph{AlignFormer} \cite{fan2025alignformer} pairs a
...
text-aligned tokens. \emph{Seed-ASR Converter} \cite{bai2024seed} learns
a context-aware transformation that improves multilingual generalization.
\emph{SALMONN}'s Q-Former \cite{tang2023salmonn} and the OSUM adapter
\cite{geng2025osum} use a small set of trainable queries to distill
semantically rich acoustic representations into the LLM's input space.
As an alternative to adapter-based bridging, X-OPD
\cite{cao2026xopd} distills text-LLM capabilities into a speech-LLM via
...
\cite{mousavi2025alas}, which uses Whisper timestamps as a reference and
...
\cite{chen2022unispeech}, which adds speaker-aware objectives to the SSL
loss; DinoSR \cite{liu2023dinosr}, which combines self-distillation with
masked acoustic modeling; and Seed-ASR \cite{bai2024seed}, which scales a
...
(GSLM) \cite{lakhotia2021generative} and SPIRIT-LM \cite{nguyen2025spirit},
...
by Whisper \cite{radford2023robust}'s 680{,}000-hour multilingual corpus,
...
later systems (OSUM \cite{geng2025osum}, SALMONN \cite{tang2023salmonn})
...
\cite{zhang2023speechgptempoweringlargelanguage} integrates discrete
speech tokens into a decoder-only LLM backbone with strong zero-shot
generalization. COSMIC \cite{pan2024cosmicdataefficientinstructiontuning}
...
limited paired data. InstructSpeech \cite{huang2024instructspeech} uses
hierarchical adapters and multi-step reasoning for fine-grained semantic
and acoustic editing. LLaMA-Omni
\cite{fang2025llamaomniseamlessspeechinteraction} pairs a frozen Whisper
...
recently, Audio-FLAN \cite{xue2025audioflan} demonstrates that scaling
...
catastrophic forgetting. HDMoLE \cite{mu2025hdmolemixtureloraexperts}
...
\cite{inoue2024elpadaptersparameterefficientadapter} insert lightweight
...
Zipper-LoRA \cite{mei2026zipperlora} further advances speech-specific
...
unpaired domain-specific text \cite{fang2025lowresourcedomainadaptationspeech}
...
performance. Decoder-only context perturbation \cite{suh2024improving}
...
quality \cite{communication2023seamlessmultilingualexpressivestreaming}.

\paragraph{Reinforcement Learning from Human Feedback (RLHF).}
RLHF \cite{ouyang2022training,christiano2017deep} trains a reward model
...
methods (typically PPO \cite{schulman2017proximalpolicyoptimizationalgorithms}
...
substitute RLHF with lighter alternatives \cite{rafailov2023direct}.

\paragraph{Direct Preference Optimization (DPO).}
DPO \cite{rafailov2023direct} eliminates the explicit reward model by
...
\cite{zhang2024speechalignaligningspeechgeneration} iteratively refines a
...
\cite{lin2025alignslmtextlessspokenlanguage} extends DPO to a textless
...
OpenOmni \cite{luo2025openomniadvancingopensourceomnimodal} introduces a
...
comparisons, Emo-LiPO \cite{lin2026emolipo} proposes listwise preference
...
RLAIF \cite{lee2024rlaifvsrlhfscaling,bai2022constitutionalaiharmlessnessai}
...
\cite{yu2024rlaifvopensourceaifeedback} extends this to multimodal models
...
AlignSLM \cite{lin2025alignslmtextlessspokenlanguage} uses semantic
...
\hline
\textbf{Annotation source} & Human preference pairs & Human preference pairs; some implementations mix synthetic data & AI-generated preference pairs & Speech requires acoustic + paralinguistic judgements; LLM judges are still moderate-correlation \cite{zheng2023judging} \\
\hline
...
default \cite{radford2023robust,bai2024seed}, but parallel speech-text
...
\cite{mousavi2025alas,fan2025alignformer}. Benchmarks such as SALMon and
