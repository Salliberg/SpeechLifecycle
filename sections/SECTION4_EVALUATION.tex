...
behavior that matters for real use. This behavioural framing follows a
...
human judgements even for a relatively bounded construct such as research
...
Table~\ref{tab:joint_metric_pilot} assembles a single worked example for
one (benchmark, model) pair. We deliberately choose the pair with the
most complete numeric coverage in our reading notes---FMSU-Bench with
its FM-Speech model, which reports a top-model accuracy and its
evaluation hardware---and we couple it with a published latency figure
drawn from a serving-side study, because no benchmark in our pool
reports accuracy and latency for the same model. The result is the
\emph{minimum viable} joint report the field currently supports: even
this best case must borrow its latency number from a different paper and
leaves safety and reproducibility only partially populated, which is
itself the operational-validity gap made visible.

\begin{table}[!ht]
\centering
\caption{\textbf{Pilot: joint accuracy--latency--governance report for
one benchmark.} As a worked example of the joint-metric reporting we
advocate, this pilot combines a published accuracy number, a published
latency/hardware setting, and a Deployment Card summary for a single
...
\toprule
Field & Value & Source & Deployment-Card entry \\
\midrule
Benchmark &
FMSU-Bench &
...
Latency &
48.99\,ms average TTFB (streaming TTS) &
\citep{su2026ultralowtts} &
SLA class: mean TTFB (no p50/p95 reported) \\
Hardware &
