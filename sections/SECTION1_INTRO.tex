...
\paragraph{Objectives and guiding questions.}
Concretely, we set out to answer four questions, one per lifecycle
stage, plus a cross-cutting one. On training, we ask which strategies
the field is actually using---from modality alignment and weakly
supervised pretraining to preference alignment via RLHF, DPO, and
RLAIF---and, more importantly, which of these choices quietly limit
what evaluation, deployment, or safety can later reveal. On
evaluation, we ask whether the current wave of Speech LLM benchmarks
measures what a practitioner needs before trusting a system in
production; the answer, developed in
Section~\ref{sec:evaluation_benchmarking}, rests on an 18-benchmark
audit across 17 capability dimensions and four validity types. On
deployment, we ask which failure modes---latency fragmentation,
streaming mismatch, memory growth, hardware mismatch, and interaction
incompleteness---current serving stacks and architectures actually
solve, and what reporting is required for practitioners to compare
them. On responsible design, we ask which risks are genuinely
speech-specific rather than inherited from text safeguards, and how
well existing defenses cover them. The cross-cutting question that
motivates the survey format is simpler to state and harder to answer:
how do choices made at one stage propagate into the others?
Table~\ref{tab:coupling_evidence} answers it with five documented
examples, and each section closes with a short recap that ties its
findings back to the same table. The survey is an audit rather than a
construction: we do not propose a new model or benchmark, but we do
argue that Speech LLMs cannot be reviewed one stage at a time.
